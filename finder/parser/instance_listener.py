from parser.PddlListener import PddlListener
from parser.ast_types import PDDLInstance, HashedLiteral
...
class InstanceListener(PddlListener):
    ...
    def __init__(self):
        ...
        self.initPredicates = set()
        self.goalPredicates = set()
    ...
    def enterInitEl(self, ctx):
        ...
        if len(nameLiteralChildren) >= 1:
            ...
            predicate = f"({atomicNameFormula.predicate().getText().lower()} {' '.join(map(str, atomicNameFormula.NAME()))})"
            hashedLiteral = HashedLiteral('+', predicate)

            if len(nameLiteralChildren) > 1 and nameLiteralChildren[1] == 'not':
                hashedLiteral.sign = '-'

            self.initPredicates.add(hashedLiteral)
    ...
    def parseLiteral(self, literal):
        ...
        if len(literalChildren) > 1 and literalChildren[1].getText().lower() == 'not':
            sign = '-'
            try:
        ...
        self.goalPredicates.add(hashedLiteral)
